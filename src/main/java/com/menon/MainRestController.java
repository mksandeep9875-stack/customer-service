...
public class MainRestController {
    ...
    public ResponseEntity<?> validateToken(@RequestHeader("Authorization") String tokenValue) { // AOP - MICROMETER
        ...
        if (token.isPresent() && token.get().getStatus().equals("ACTIVE")) {
            ...
            if (credential.isPresent()) {
                ...
                Principal principal = createPrinciple(token, credential);
    ...
    private Principal createPrinciple(Optional<Token> token, Optional<Credential> credential) {
    ...
    private ResponseEntity<Principal> getPrincipalResponseEntityForInvalidToken() {
